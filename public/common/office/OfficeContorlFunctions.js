...
function intializePage(fileUrl) {
    ...
    OFFICE_CONTROL_OBJ.DoHandSign2("匿名用户", "", 0, 0, 0, 100);
    //doHandSign();
}
function intializeWebofficePage(fileUrl, trackRevision, toolbar) {
    OFFICE_CONTROL_OBJ = document.all("TANGER_OCX");
    NTKO_OCX_OpenDoc(fileUrl);

    var toolbarDom = $(".toolbar");
    toolbarDom.click(function () {
    ...
    var trackbarDom = $(".trackRevision");
    trackbarDom.click(function () {
        if ($(this).hasClass("open")) {
            $(this).removeClass("open");
            $(this).html("打开WORD痕迹");
            OFFICE_CONTROL_OBJ.ActiveDocument.TrackRevisions = false;
        } else {
            $(this).addClass("open");
            $(this).html("关闭WORD痕迹");
    ...
    if (toolbar == false) {
        ...
        toolbarDom.addClass("open");
        toolbarDom.html("关闭WORD工具栏");
    }

    if (trackRevision == true) {
        $(".trackRevision").addClass("open");
        $(".trackRevision").html("关闭WORD留痕");
        //OFFICE_CONTROL_OBJ.ActiveDocument.TrackRevisions = trackRevision;
    }
